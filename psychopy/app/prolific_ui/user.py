...
from .functions import logInProlific, logOutProlific
...
class UserEditor(wx.Dialog):
    ...
    def __init__(self, parent=None, id=wx.ID_ANY, style=defStyle,
        ...
        self.parent = parent
        self.app = wx.GetApp()
        pavSession = prolific.getCurrentSession()
        if pavSession.user:
            pavSession.gitlab.auth()
            self.user = pavSession.user
        else:
            self.user = logInProlific(parent=parent)
            if not self.user:
                return  # they were given a login but cancelled
        if type(self.user) != prolific.User:
            self.user = prolific.User(gitlabData=self.user)
    ...
    def submitChanges(self, evt=None):
        # print("updating gitlab.prolific.co users from within PsychoPy not yet working")
        # isDirty = False
    ...
    def onSetAvatar(self, event=None):
        print("Uploading a user image (avatar) from within PsychoPy is not yet supported. "
              "You can do that by going to the gitlab.prolific.co profile settings.")
