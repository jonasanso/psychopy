...
from ._base import ProlificMiniBrowser, ProlificCommitDialog
from psychopy.projects import prolific  # NB prolific will set gitpython path
...
def logInProlific(parent, event=None):
    """Opens the built-in browser dialog to login to prolific
    ...
    dlg = ProlificMiniBrowser(parent=parent, loginOnly=True)
    dlg.ShowModal()  # with loginOnly=True this will EndModal once logged in
    if dlg.tokenInfo:
        token = dlg.tokenInfo['token']
        prolific.login(token, rememberMe=True)  # log in to the current prolific session
        return prolific.getCurrentSession().user


def logOutProlific(parent, event=None):
    """Opens the built-in browser dialog to login to prolific
    ...
    dlg = ProlificMiniBrowser(parent=parent, logoutOnly=True)
    dlg.logout()
    dlg.Destroy()
...
def showCommitDialog(parent, project, initMsg="", infoStream=None):
    ...
    dlg = ProlificCommitDialog(parent, id=wx.ID_ANY, title=_translate("Committing changes"), changeInfo=changeInfo)
...
def noGitWarning(parent):
    ...
    msg = wx.StaticText(dlg, label=_translate(
            "You need to install git to use Prolific projects"))
