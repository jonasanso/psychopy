...
class ProlificMiniBrowser(wx.Dialog):
    ...
    def __init__(self, parent, user=None, loginOnly=False, logoutOnly=False,
        ...
        if not user:
            self.user = prolific.getCurrentSession().user
        if not user:
            self.login()
        if not user:
            return None
    ...
    def logout(self):
        ...
        self.browser.LoadURL('https://gitlab.prolific.co/users/sign_out')

    def login(self):
        self._loggingIn = True
        authURL, state = prolific.getAuthURL()
    ...
    def gotoProjects(self):
        self.browser.LoadURL("https://prolific.co/projects.html")
    ...
    def checkForLoginURL(self, event):
        ...
        if 'access_token=' in url:
            ...
            prolific.login(self.tokenInfo['token'])
            if self.loginOnly:
                self.EndModal(wx.ID_OK)
        elif url == 'https://test-client.prolific.co':
            # this is what happens if the user registered instead of logging in
            # try now to do the log in (in the same session)
            self.login()
        elif url == 'https://test-client.prolific.co/studies/drafts':
            # this is what happens if the user registered instead of logging in
            # try now to do the log in (in the same session)
            self.login()
        else:
    ...
    def checkForLogoutURL(self, event):
        url = event.GetURL()
        if url == 'https://gitlab.prolific.co/users/sign_in':
            if self.logoutOnly:
                self.EndModal(wx.ID_OK)
...
class ProlificCommitDialog(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ProlificCommitDialog, self).__init__(*args, **kwargs)
